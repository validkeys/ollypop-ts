...
export class FileScanner {
  ...
  async scanSources(sources: SourceConfig[], excludePatterns: string[] = []): Promise<FileInfo[]> {
    ...
    const uniqueFiles = allFiles.filter(
      (file, index, array) => array.findIndex((f) => f.path === file.path) === index
    );

    return uniqueFiles;
  }

  private async scanSource(source: SourceConfig, excludePatterns: string[]): Promise<FileInfo[]> {
    const {
      path: sourcePath,
      recursive = false,
      maxDepth,
