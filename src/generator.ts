...
export class BarrelGenerator {
  ...
  private async shouldWriteFile(
    ...
    try {
      // If existing content wasn't provided, read it from the file
  ...
  private async writeWithMarkers(
    ...
    try {
      ...
      if (fileExists && existingContent.includes(startMarker)) {
        ...
        // Create new file with markers or add markers to existing file
        const baseContent = fileExists ? existingContent : '';
        updatedContent = [
          baseContent.trimEnd(),
          baseContent ? '' : '', // Add blank line only if there's existing content
          startMarker,
          '',
          content.trim(),
          '',
