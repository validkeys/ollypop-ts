...
export class BarrelGenerator {
  ...
  async generateBarrel(definition: BarrelDefinition): Promise<void> {
    ...
    if (!definition.output?.trim()) {
      throw new Error(
        `Barrel '${definition.name}': output path cannot be empty`
      )
    }
  ...
  private async generateVariableBasedBarrel(
    definition: VariableBarrelDefinition
  ): Promise<void> {
